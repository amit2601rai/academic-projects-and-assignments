...
<%@ page import="elites.model.*" %>
  ...
  <center><table width="892" border="0" height="55">
        <tbody><tr>
        ...
        <tr>
          <td height="214"><table width="708" height="200" border="0">
            <tr>
              ...
              <%try
{String temp_mgid=(String)session.getAttribute("mgid");
 String temp_courtno=(String)session.getAttribute("courtno");
 DbModel dm=new DbModel();%>
  
<span class="head">view and provide/deny arrest warrant</span>  <br><br><br>
              <table width="702" border="1" cellspacing="0" bordercolor="#000000">
                <tr>
                  <td width="43" class="para3">copid</td>
                  <td width="94" class="para3"> police station no </td>
                  <td width="72" class="para3">firno</td>
                  <td width="85" class="para3"> date</td>
                  <td width="98" class="para3"> against whom</td>
                  <td width="89" class="para3"> reason/details</td>
                  <td width="71" colspan="2">&nbsp;</td>
                  
                </tr>
                <%//arrest warrant list
System.out.print("jsp");
String str="select * from ELITE.ARRESTWARRANT where courtno='"+temp_courtno+"' and provided='false' " ;
ResultSet rs2=dm.executeQuery(str);
while(rs2.next())
{
%>
                <tr>
                  <td class="para1"><%=rs2.getString(1)%></td>
                  <td class="para1"><%=rs2.getString(2)%></td>
                  <td class="para1"><%=rs2.getString(3)%></td>
                  <td class="para1"><%=rs2.getString(4)%></td>
                  <td class="para1"><%=rs2.getString(5)%></td>
                  <td class="para1"><%=rs2.getString(6)%></td>
                  <td><a href="/elites/warrantaccept.do?refno=<%=rs2.getString(3)%>">accept</a></td>
                  <td><a href="/elites/warrantdelete.do?refno=<%=rs2.getString(3)%>">delete</a></td>
                </tr>
               <% } %> </table> <%
}
catch(Exception e)
